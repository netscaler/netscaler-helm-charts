...
- Following configurations needs to be done on the NetScaler's
  ...
  - For static proximity, the location database has to be applied externally
    ```
    add locationfile /var/netscaler/inbuilt_db/NetScaler_Netscaler_InBuilt_GeoIP_DB_IPv4
    ```
...
| gslb.imageTag                  | Optional  |  `1.35.6`               |   The NetScaler ingress controller image tag            | 
...
| gslb.logLevel | Optional | DEBUG | The loglevel to control the logs generated by NSIC. The supported loglevels are: CRITICAL, ERROR, WARNING, INFO, DEBUG and TRACE. For more information, see [Logging](https://github.com/netscaler/netscaler-k8s-ingress-controller/blob/master/docs/configure/log-levels.md).|
