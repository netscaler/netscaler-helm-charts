# Deploy NetScaler as an Ingress Gateway in Istio environment using Helm charts
...
To deploy NetScaler VPX or MPX with secret volume, do the following step:
        ...
        helm install my-release netscaler/citrix-adc-istio-ingress-gateway --namespace netscaler-system --set ingressGateway.EULA=YES,secretName=nslogin,ingressGateway.netscalerUrl=https://<nsip>[:port],ingressGateway.vserverIP=<IPv4 Address>,ingressGateway.secretVolumes[0].name=test-ingressgateway-certs,ingressGateway.secretVolumes[0].secretName=test-ingressgateway-certs,ingressGateway.secretVolumes[0].mountPath=/etc/istio/test-ingressgateway-certs
...
Metrics Exporter requires the IP address of NetScaler CPX or VPX Ingress Gateway. It is retrieved from the value specified for `ingressGateway.netscalerUrl`.
...
To deploy NetScaler VPX or MPX with NetScaler certificate verification, do the following step:
        ...
        helm install citrix-adc-istio-ingress-gateway netscaler/citrix-adc-istio-ingress-gateway --namespace netscaler-system --set ingressGateway.EULA=YES,secretName=nslogin,ingressGateway.netscalerUrl=https://<nsip>[:port],ingressGateway.vserverIP=<IPv4 Address>,ingressGateway.adcServerName=<NetScaler Cert Server Name>
...
| `xDSAdaptor.imageRegistry`                | Image registry of the NetScaler xDS adaptor container(Refer compatibility matrix)               | `quay.io`               |     Mandatory                  |
| `xDSAdaptor.imageRepository`              | Image repository of the NetScaler xDS adaptor container(Refer compatibility matrix)               | `citrix/citrix-xds-adaptor`               |    Mandatory                  |
| `xDSAdaptor.imageTag`                     | Image tag of the NetScaler xDS adaptor container(Refer compatibility matrix)               | `0.10.3`               |   Mandatory                  |
...
| `coe.coeURL`          | Name of [NetScaler Observability Exporter](https://github.com/netscaler/netscaler-observability-exporter) Service in the form of "<servicename>.<namespace>"  | null            | Optional|
...
| `ADMSettings.ADMIP`          | NetScaler Application Delivery Management (ADM) IP address  | null            | Mandatory for NetScaler CPX |
| `ADMSettings.licenseServerIP`          | NetScaler License Server IP address  | null            | Optional |
| `ADMSettings.licenseServerPort` | NetScaler ADM port if a non-default port is used                                                                                        | 27000                                                                 | Optional|
...
| `ingressGateway.netscalerUrl`       | URL or IP address of the NetScaler which Istio-adaptor configures (Mandatory if citrixCPX=false)| null   |Mandatory for NetScaler MPX or VPX|
| `ingressGateway.vserverIP`       | Virtual server IP address on NetScaler (Mandatory if citrixCPX=false) | null | Mandatory for NetScaler MPX or VPX|
| `ingressGateway.adcServerName`          | NetScaler ServerName used in the NetScaler certificate  | null            | Optional |
| `ingressGateway.imageRegistry`                   | Image registry of NetScaler CPX designated to run as Ingress Gateway              | `quay.io`               |     Mandatory for NetScaler CPX                  |
| `ingressGateway.imageRepository`                 | Image repository of NetScaler CPX designated to run as Ingress Gateway              | `citrix/citrix-k8s-cpx-ingress`               |    Mandatory for NetScaler CPX                  |
| `ingressGateway.imageTag`                  | Image tag of NetScaler CPX designated to run as Ingress Gateway              | `13.1-30.52`               |   Mandatory for NetScaler CPX                  |
...
| `ingressGateway.mgmtHttpPort`      | Management port of the NetScaler CPX                                                                                              | 9080                                                                  | Optional|
| `ingressGateway.mgmtHttpsPort`    | Secure management port of NetScaler CPX                                                                                           | 9443                                                                  | Optional|
